...
";
    ...
    public string GetCategoryName(string categoryNo)
    {
        ...
        var model = repo.ReadSingle(m => m.CategoryNo == categoryNo);
        if (model != null) str_category_name = model.CategoryName;
    ...
    public List<Categorys> GetShopCategoryList()
    {
        ...
        {
            var model = repo.ReadAll(m => m.IsEnabled == true).OrderBy(m => m.CategoryNo).ToList();
            foreach (var item in model)
            {
                item.ProductCounts = prod.repo.ReadAll(m => m.CategoryNo == item.CategoryNo).Count();
            }
            return model;
        }
    }
    public List<Categorys> GetShopCategory1()
    {
        using (z_repoProducts prod = new z_repoProducts())
        {
            var model = repo.ReadAll(m =>
                m.IsEnabled == true && m.ParentNo == "")
                .OrderBy(m => m.SortNo).ToList();
            return model;
        }
    }

    public List<Categorys> GetShopCategory2(string categoryNo)
    {
        using (z_repoProducts prod = new z_repoProducts())
        {
            var model = repo.ReadAll(m =>
                m.IsEnabled == true && m.ParentNo == categoryNo)
                .OrderBy(m => m.SortNo).ToList();
            foreach (var item in model)
            {
                item.ProductCounts = prod.repo.ReadAll(m => m.CategoryNo == item.CategoryNo).Count();
